package com.tailoris.community.serverless;

import com.tailoris.community.CommunityApplication;
...
/**
 * 社区服务 Serverless 函数入口处理器
 *
 * 用于在阿里云函数计算 (FC) / 腾讯云 SCF 中运行 Spring Boot 应用。
 * 通过轻量级容器封装整个 Spring Boot 上下文，实现 HTTP 请求转发。
 *
 * 使用方式：
 *  - Handler 配置: com.tailoris.community.serverless.CommunityFunctionHandler::handleRequest
 *  - 运行时: custom-container (推荐) 或 java11/java17
 */
public class CommunityFunctionHandler {
    ...
    private synchronized void ensureInitialized() {
        ...
        // 启动 Spring Boot 应用
        SpringApplicationBuilder builder = new SpringApplicationBuilder(CommunityApplication.class)
